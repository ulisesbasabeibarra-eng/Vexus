...
void salir() { 
    if (digitalRead(T1)) { 
        ...
        while (millis() < tiempo_inicio + 1200) {
...
void loop() {
   ...
   bool borde_atra = digitalRead(T3); //TRUE si detecta línea blanca
    ...
    if (lectura(S3) > 700){ // sensor del medio       
        MODOS = ATACAR;                                     
    }                                                          
     /*else if (lectura(S2) <= DISTANCIA){//sensor der 45
        MODOS = BUSCAR_DE45;
    }*/
    else if (lectura(S4) > 700){//sensor izq 45
        MODOS = BUSCAR_IZQ45;
    }
    else if (lectura(S1) > 700){ // sensor der       
        MODOS = BUSCAR_DE;                                    
    }                                                      
    else if (lectura(S5) > 700){ // sensor izq     
        MODOS = BUSCAR_IZ;                                
    }
    else {
        MODOS = BUSCAR;
    ...
    SerialBT.print("MODO: ");
    SerialBT.println(MODOS);
/*
    SerialBT.print("medio: ");
    SerialBT.print(lectura(S3));
    SerialBT.print(", der45: ");
    SerialBT.print(lectura(S2));
    SerialBT.print(", izq45: ");
    SerialBT.print(lectura(S4));
    SerialBT.print(", izq: ");
    SerialBT.print(lectura(S5));
    SerialBT.print(", der: ");
    SerialBT.println(lectura(S1));

    
    SerialBT.print("Der: ");
    SerialBT.print(borde_der);
    SerialBT.print(", Izq: ");
    SerialBT.print(borde_izq);
    SerialBT.print(", Atras: ");
    SerialBT.println(borde_atra);
*/
    switch (MODOS){
    case BUSCAR:
    ...
    case ATACAR:
        motores(VMAX_IZQ, VMAX_DER);
        {
            ...
            // Si detecta algo a la izquierda y es el objeto más cercano hasta ahora
            ...
            // Si detecta algo a la derecha y es más cercano que lo memorizado
        ...
        if (borde_der || borde_izq ) {
            if (objetivo_memorizado) {
                ...
                // Si no vio nada, simplemente aplica la funcion de salir normal
                MODOS = ATRAS;
            }
        }
    ...
    case RETROCEDER_MEMORIA:
        motores(-VMAX_IZQ, -VMAX_DER); // retrocede a la misma velocidad base
